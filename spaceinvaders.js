...
// optimisation pour autres services
...
function gameLoop(){
  ...
  //jeu gagné en tuant les ennemis
  if(aliveInvaders == 0){
    gameRunning=false;
...
function drawArmyOfInvaders(){
  for (let i = 0; i < armyRows; i++) {
    for(let j = 0; j < armyColumns; j++){
        let soldier = armyArray[i][j];
        if (soldier.status=='alive') {
          // dessins des Invader(soldier.x,soldier.y,invaderSpriteHeight);
...
function drawBullet(bx,by){
  ctx.beginPath();
  ctx.beginPath();       //  Commencer un nouveau path
  ctx.moveTo(bx, by);    // Déplacez le pen vers (30, 50)
  ctx.lineTo(bx, by-bullet__height);  // Tracez une ligne (150, 100)
...
function drawScoreSeprateLine(){
  ctx.beginPath();
  ctx.beginPath();       // Commencer un nouveau path
  ctx.moveTo(0, canvas.height- (scoreBarHeight-15));    // Déplacez le pen vers (30, 50)
  ctx.lineTo(canvas.width, canvas.height - (scoreBarHeight-15));  //Tracez une ligne (150, 100)
  ctx.lineWidth = 2;
  ctx.shadowBlur = 5;
  ctx.shadowOffsetY=1;
  ctx.shadowOffsetY=-1;
...
function drawInvaderBullet(ix, iy){
  ctx.beginPath();
  ctx.beginPath();       // Commencer un nouveau path
  ctx.moveTo(ix, iy);    // Déplacez le pen vers (30, 50)
  ctx.lineTo(ix, iy+bullet__height);  // Tracez une ligne (150, 100)
